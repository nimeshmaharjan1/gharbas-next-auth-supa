...
interface Props {
  ...
  accept?: string;
  sizeLimit?: number;
  onChangePicture: (reader: any) => any;
}
const ImageUpload: React.FC<Props> = ({
  ...
  const pictureRef = useRef() as RefObject<HTMLInputElement>;

  const [image, setImage] = useState(initialImage);
  const [updatingPicture, setUpdatingPicture] = useState(false);
  const [pictureError, setPictureError] = useState(null) as any;
    ...
    <div className="flex flex-col space-y-2">
      ...
      <button
        ...
        className={classNames(
          ...
          image?.src
            ? "hover:opacity-50 disabled:hover:opacity-100 h-full"
